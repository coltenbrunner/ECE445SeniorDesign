"""All credit for this script goes to https://github.com/Moddy2024/Bird-Classification where I am using this as a blackbox minus a minor alteration to have matplotlib exported to another file and saved the time it takes to predict on a bird"""
...
model=(BirdResnet())
#model.load_state_dict(torch.load('/content/drive/bird-classification/bird-resnet34best4.pth',map_location=torch.device(device)))
model.load_state_dict(torch.load('/Users/coltenbrunner/Desktop/ECE445_BirdCAM/Bird-Classification/trained-models/bird-resnet34best.pth',map_location=torch.device(device)))
...
"""def predict_image(path, model):
    ...
    plt.show()
"""
def predict_image(path, model, counter, saveDirectory):
    ...
    _, preds  = torch.max(yb, dim=1)
    # Retrieve the class label
    prediction = bird_name_map.get(preds[0].item())
    elapsed_time = time.time() - start_time  # Calculate elapsed time
    accuracy = str(round(torch.max(yb).item(), 4)*100)+'%'
    print('Predicted:', prediction, 'with a probability of', str(round(torch.max(yb).item(), 4)*100)+'%')
    print('Elapsed Time:', elapsed_time, 'seconds')  # Print elapsed time
    
    return prediction, accuracy, elapsed_time, img
